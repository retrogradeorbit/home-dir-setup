...
-- This is setup for 1920x1080 monitor
Config {
    font = "xft:Fixed-12",
    bgColor = "#000000",
    fgColor = "#ffffff",
    ...
    commands = [
        ...
        Run Battery ["-t","Batt: % / ","-L","25","-H","75","-h","green","-n","yellow","-l","red"] 10,
        Run StdinReader
    ],
    sepChar = "%",
    alignSep = "}{",
    template = "%StdinReader% }{ %multicpu%   %memory%   %swap%   %em1%   <fc=#FFFFCC>%date%</fc>   %KPAO%  %battery%"
}
